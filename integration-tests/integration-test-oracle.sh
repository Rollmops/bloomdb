...
# BloomDB Oracle Integration Test Script
# Tests all functionalities: destroy, baseline, migrate, repair, etc.
#
# Usage: ./integration-test-oracle.sh [BASELINE_VERSION]
...
DB_USER="bloomdb"
DB_PASSWORD="bloomdb"
CONN_STRING="oracle://$DB_USER:$DB_PASSWORD@$DB_HOST:$DB_PORT/$DB_SERVICE"
...
# Function to run sqlplus commands
run_sqlplus() {
    docker exec bloomdb-oracle-test bash -c "echo \"$1\" | sqlplus -s $DB_USER/$DB_PASSWORD@$DB_SERVICE"
}
...
until docker exec bloomdb-oracle-test bash -c "echo 'SELECT 1 FROM DUAL;' | sqlplus -s $DB_USER/$DB_PASSWORD@$DB_SERVICE" | grep -q "1"; do
...
run_sqlplus "SELECT * FROM BLOOMDB_VERSION ORDER BY \"installed rank\";"
echo ""
...
echo "BLOOMDB_VERSION table contents:"
run_sqlplus "SELECT * FROM BLOOMDB_VERSION ORDER BY \"installed rank\";" 2>/dev/null || echo "Table does not exist (expected after destroy)"
echo ""

print_separator "Test 23: Verify database is empty"
print_status "Test 23: Verifying database is empty after destroy"
if run_sqlplus "SELECT table_name FROM user_tables WHERE table_name IN ('USERS', 'POSTS', 'FAULTY_TABLE');" | grep -q "USERS\|POSTS\|FAULTY_TABLE"; then
